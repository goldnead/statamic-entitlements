 ...
 * contract copied byte for byte, and without this file nothing would check that
 * claim. The sibling is neither a `require` nor a `require-dev`, so its
 ...
 * `Metric` would therefore leave every test green and fatal on the first
 ...
 *
 * **The base class is held to a stricter standard than the contracts.** It used
 * to be compared here by reflection like everything else, and that comparison
 * was blind in exactly the place it mattered: the metrics in this package had
 * inlined `TableMetric::inPeriod()` instead of calling it, the original was
 * repaired three times over, and every signature still matched. Only the bodies
 * differed. So `tests/Fakes/insights-table-metric.php` is a file of its own now
 * and is compared character for character.
 *
 * So the copy is held against the original wherever the original can be found:
 * installed in `vendor/`, or checked out beside this package, which is how this
 * family is developed. Where it cannot be found the test skips and says why — a
 * machine without the sibling cannot answer the question, and pretending
 ...
 *
 * Extends PHPUnit's own case rather than this package's: it boots no
 * application, touches no database and asks nothing of Laravel. A test bed for
 * a `shell_exec` and a `json_decode` would only make it slower.
 */
class InsightsContractsMatchTest extends TestCase
{
    /** The interfaces a metric here implements or is read through. */
    protected const VERTRAEGE = ['Contracts\Metric', 'Contracts\HasBreakdowns'];
     ...
     *
     * The bucket grain is compared against `BUCKET_MONTH` when a series is
     * built, and `unit()` returns these strings straight to a screen that
     * formats by them. A renamed value upstream would silently turn every
     * monthly chart daily, or every count into something else.
     */
     ...
     * `TableMetric` is not a signature, it is behaviour. It refuses a row whose
     * timestamp is null, compares the upper bound half-open because a binding
     * truncates the fraction of a second, truncates a timestamp differently on
     * SQLite, MySQL and Postgres, and narrows by brand. Every one of those is
     * invisible to reflection — which is how a metric in this package went on
     * carrying its own copy of a window that had been repaired upstream three
     * times, with a green suite the whole way.
     *
